...
public class AuthController : Controller
{
    ...
    [HttpGet]
    public IActionResult CandidateRegister() => View();

    [HttpGet]
    public IActionResult RecruiterRegister() => View();

    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> CandidateLogin(LoginViewModel vm, string? returnUrl = null)
    {
        ...
        if (hasError)
        {
            ...
            return RedirectToAction(nameof(CandidateLogin));
        }
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> RecruiterLogin(LoginViewModel vm, string? returnUrl = null)
    {
        var email    = vm.Email?.Trim() ?? "";
        var password = vm.Password?.Trim() ?? "";
        bool hasError = false;
        ...
        {
            TempData["PrefillEmail"] = email;
            return RedirectToAction(nameof(RecruiterLogin));
        }
    ...
    [HttpGet]
    public async Task<IActionResult> RecruiterLogout()
    {
        await HttpContext.SignOutAsync("RecruiterCookies");
    ...
    public async Task<IActionResult> AdminLogout()
    {
        ...
        return RedirectToAction("CandidateLogin", "Auth");
    }
    ...
    public async Task<IActionResult> GoogleCallback()
    {
        ...
        if (user is not null)
        {
            ...
            if (user.IsActive != true)
            {
                ...
                return from == "recruiter"
                    ? Redirect("/Auth/RecruiterLogin")
                    : RedirectToAction(nameof(CandidateLogin));
            }

            if (from == "recruiter" && user.UserType == "Candidate")
            {
                TempData["GeneralError"] = "Tài khoản ứng viên không thể đăng nhập tại trang nhà tuyển dụng.";
                return Redirect("/Auth/RecruiterLogin");
            }
        ...
        return RedirectToAction(nameof(GoogleRecruiterInfo));
    }

    [HttpGet]
    public IActionResult GoogleRecruiterInfo()
    {
        var email = HttpContext.Session.GetString(KeyGoogleEmail);
        if (string.IsNullOrEmpty(email)) return Redirect("/Auth/RecruiterLogin");

        return View(new GoogleRecruiterInfoViewModel
        {
            Email    = email,
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> GoogleRecruiterInfo(GoogleRecruiterInfoViewModel vm)
    {
        var email    = HttpContext.Session.GetString(KeyGoogleEmail);
    ...
    private async Task SignInAsync(UserAccount user, bool rememberMe, string? googleAvatar = null)
    {
        ...
        {
            "Recruiter" => ("RECRUITER",  "RecruiterCookies"),
