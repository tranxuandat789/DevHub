...
;WITH Jobs AS (
    SELECT  job_id, status, approved_at, created_at,
            ROW_NUMBER() OVER (ORDER BY job_id) - 1 AS jrn
    FROM    dbo.job_post
    WHERE   recruiter_id IN (4, 5)
      AND   status IN ('APPROVED', 'CLOSED')
),
Cands AS (
    SELECT  candidate_id,
            ROW_NUMBER() OVER (ORDER BY candidate_id) - 1 AS crn,
            COUNT(*) OVER () AS cnt
    FROM    dbo.candidate
),
Slots AS (SELECT 0 AS slot UNION ALL SELECT 1 UNION ALL SELECT 2)  -- 3 ứng viên / job
SELECT
    j.job_id,
    c.candidate_id,
    j.approved_at,
    ...
    CASE
        WHEN j.status = 'APPROVED'
            THEN CASE s.slot WHEN 0 THEN 'PENDING' WHEN 1 THEN 'APPROVED' ELSE 'REJECTED' END
        ELSE     CASE s.slot WHEN 0 THEN 'HIRED'   WHEN 1 THEN 'FINISHED' ELSE 'REJECTED' END
    END AS app_status,
    ROW_NUMBER() OVER (ORDER BY j.job_id, s.slot) - 1 AS arn,   -- để chia đều CV
    CAST(NULL AS NVARCHAR(255)) AS cv_title,
    CAST(NULL AS NVARCHAR(500)) AS cv_url
INTO #Map
FROM Jobs j
CROSS JOIN Slots s
...
FROM #Map m
JOIN (VALUES
    (0, N'/uploads/cvs/1_633d6964-5424-4f62-ae1c-f026d9c518ad.docx'),
    (1, N'/uploads/cvs/23_5486f90b-dcfe-436e-8012-84348cbd697f.docx'),
    (2, N'/uploads/cvs/23_88027a26-c81a-4c0d-a981-efb1a50d826b.pdf'),
    (3, N'/uploads/cvs/23_e8626748-26b4-4859-9af0-5c183050a74f.pdf'),
    (4, N'/uploads/cvs/24_52e163f8-d099-4fc4-81c7-a57f19fe27c9.docx')
...
SELECT m.candidate_id, m.cv_title, m.cv_url, N'Kỹ năng theo hồ sơ ứng viên', 0
FROM   #Map m;
...
-- ============================================================
INSERT INTO dbo.application (job_id, candidate_id, cv_id, cover_letter, status, applied_at)
SELECT  m.job_id,
        m.candidate_id,
        cv.cv_id,
        N'Tôi rất quan tâm tới vị trí này và mong muốn được đóng góp cho công ty.',
        m.app_status,
        DATEADD(DAY, -(m.slot + 1), ISNULL(m.approved_at, m.created_at))
FROM    #Map m
JOIN    dbo.cv cv ON cv.candidate_id = m.candidate_id AND cv.title = m.cv_title;
...
JOIN (
    SELECT job_id, COUNT(*) AS cnt FROM dbo.application GROUP BY job_id
) x ON x.job_id = jp.job_id
WHERE   jp.recruiter_id IN (4, 5)
...
-- ============================================================
SELECT  cv.cv_url, COUNT(*) AS total_applications
FROM    dbo.application a
JOIN    dbo.job_post j ON j.job_id = a.job_id
